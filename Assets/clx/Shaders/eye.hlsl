...
float4 PbrPixelShading(in VertexShadingOutput IN):SV_Target
{
 ...
 (DirectLight2)*=(((((((((shadow)*(SunColor2.rgb)))*(Diffuse.a)))*(20)))*(cEyeColor.rgb)));//DirectLight2
 float3 virtualNormalize=normalize(cVirtualLitDir.xyz);//VirtualLitDir
 float3 VirtualLightColor=((((cVirtualLitColor.rgb)*(UserData[1].z)))*(2));
 float3 VirtualLight=saturate(dot(N,virtualNormalize));//VirtualLitNoL
 ...
 (VirtualLight2)*=(((((((VirtualLightColor)*(Diffuse.a)))*(10)))*(cEyeColor.rgb)));//VirtualLight2
