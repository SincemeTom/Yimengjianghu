Shader "GEffect/CLX_Character_Hair_Transform"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				fixed NdotV = max(0.00,clamp(dot(viewDir,normalVec),0,1));
		...
		//((diffuse_42 * diffuse_42) * (SunColor.xyz * shadow_11));
				...
				half s_den = max (0.001, 31.15926 * Roughness_X * Roughness_Y * sqrt(NdotL * NdotV));
				...
				//Virtual Spec
				//cVirtualLitDir = half4(lightDir.xyz,0);
				//cVirtualLitColor = _LightColor0;
