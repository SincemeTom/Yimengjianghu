...
float4 SSSNewShading(in VertexShadingOutput IN,float3 userData)
{
 ...
 {
 float4 DetailValue=tDetailNormalMap.Sample(sDetailNormalSampler,((IN.TexCoord.xy)*(cDetailUVScale)));//detialNormalMap
 float3 temp=float3(((((DetailValue.b)*(2)))-(1)),((((DetailValue.a)*(2)))-(1)),0);//detailValue
 (temp)=(normalize(((tangentNormal.xyz)+(((((((temp)*(0.2)))*(maokong_intensity)))*(tangentNormal.w))))));
 (DetailNormal)=(normalize(((((((IN.WorldTangent.xyz)*(temp.x)))+(((IN.WorldBinormal.xyz)*(temp.y)))))+(((IN.WorldNormal.xyz)*(temp.z))))));//DetailNormalVec
 (SpecularMask)=(lerp(half3(0.5,0.5,0.5),DetailValue.rrr,tangentNormal.w));//SpecularMask
 }
 }
 float3 BentNormal=N;
