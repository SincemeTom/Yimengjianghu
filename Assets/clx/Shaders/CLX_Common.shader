	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				GILighting.xyz = linearColor.xyz * ShadowColor.x * (10.0 + cPointCloudm[isNegative.y + 2].w * ShadowColor.z * 100.0);
				GILighting.w = texM.z;
				...
				//skin profile
				half2 skin_uv = half2(NdotL * 0.5 + 0.5 , clamp (Alpha, 0.0, 1.0));
				
				fixed4 tex_Skin = tex2D(_SkinProfileMapSampler, skin_uv);
		...
		Pass
		{
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				half metalic = texM.y;
				//SpecularColor_10 = float(0.04, 0.04, 0.04) * (1.0 - metalic) + BaseColor * metalic;
				half3 SpecularColor = float3(0.04, 0.04, 0.04) * (1.0 - metalic).xxx + BaseColor * metalic.xxx;
				half3 DiffuseColor = (BaseColor - BaseColor * metalic) / 3.141593;
				//DiffuseColor = BaseColor / 3.141593;
				fixed NdotV = clamp(dot(viewDir,normalVec),0,1);
